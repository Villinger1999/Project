...
log_table = pd.DataFrame(dict(y_true_l=df_cohorts_2['Frustrated'], y_pred_l=pred_cohort_2_log))
log_table['log_tf'] = log_table.y_true_l == log_table.y_pred_l
log_tf = log_table.log_tf.value_counts()

base_table = pd.DataFrame(dict(y_true_b=df_cohorts_2['Frustrated'], y_pred_b=pred_cohort_2_baseline))
base_table['base_tf'] = base_table.y_true_b == base_table.y_pred_b
base_tf = base_table.base_tf.value_counts()

dt_table = pd.DataFrame(dict(y_true_dt=df_cohorts_2['Frustrated'], y_pred_dt=pred_cohort_2_decision))
